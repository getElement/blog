    ...
    mongodb.open(function (err, db) {
        ...
        db.collection('posts', function (err, collection) {
            ...
            collection.findOne({
                ...
                //如果有reprint_from即该文章是转载来的，先保存下来 reprint_from
                var reprint_from = "";
...
Post.getTag = function (tag, callback) {
    mongodb.open(function (err, db) {
        ...
        db.collection('posts', function (err, collection) {
            ...
            //查询所有tags数组内包含tag的文档并返回只含有name,time,title组成的数组
            collection.find({
                "tags": tag
            }, {
                "name": 1,
    ...
    mongodb.open(function (err, db) {
        ...
        db.collection('posts', function (err, collection) {
            ...
            collection.findOne({
                ...
                //更新被转载的原文档的reprint_info在内的reprint_to
                collection.update({
                    "name": reprint_from.name,
                    "time.day": reprint_from.day,
                    "title": reprint_from.title
